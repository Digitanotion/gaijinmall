...
if ($adStatistics['status'] == 1) {
  ...
  $decline = $adStatistics['message']['declined'];
...
<body class="hold-transition sidebar-mini">
  ...
  <div style="position:absolute; z-index:999999; bottom: 0; left:2%;" class="toast" role="alert" aria-live="assertive" aria-atomic="true">
  ...
  <div class="wrapper">
              ...
              <div class="card">
                ...
                <form action="" method="post">
                  <div class="card-body">
                    <div class="form-group">
                      <label for="exampleInputEmail1">Category Name</label>
                      <input type="text" class="form-control" id="txtUptCategName" name="txtUptCategName" value="<?php $getCateg = $adManager->getCategByID($_GET['clid']);
                                                                                                                  echo ($getCateg['status'] == "1") ? $getCateg['message'][0]['mallCategName'] : ""; ?>" placeholder="Insert a name">
                    </div>
                    <div class="form-group">
                      <label>Category Parent</label>
                            ...
                            <td>
                              <?php if ($getAllCategOptions['status'] === 1) {
                                foreach ($getAllCategOptions['message'] as $valueOpt) { ?>
                                  <span class="badge badge-info"><?php $optionData = explode("-", $valueOpt['mallCategParamName']);
                                                                  echo $optionData[1]; ?></span>
                              <?php }
                              } ?>
                            </td>
            ...
            <div class="card">
                ...
                <div class="row justify-content-center">
                  ...
                  <div class="col-12 col-sm-6 col-md-3">
                    <div class="info-box">
                      ...
                      <div class="info-box-content">
                        <span class="info-box-text">Declined Ads</span>
                        <span class="info-box-number">
                          <?php echo $decline; ?>
                          <small></small>
                        </span>
              ...
              <div class="card-body">
                <table id="adList" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    $allAds = $adManager->getAd();
                    if ($allAds['status'] == 1) {
                      $count = 1;
                      foreach ($allAds['message'] as $ad) {
                        $adCategory = $adManager->getCategoryByID($ad['mallCategID']);
                        $defaultColID = $count;
                        $mallUsrID = $ad['mallUsrID'];

                        $userDetailss = $accManager->getUserDetails($mallUsrID);
                        $defColID = $userDetailss['message']['defaultUsrID'];

                        $adTitle = $ad['mallAdTitle'];
                        $adPrice = CURRENCY . $ad['mallAdPrice'];
                        $adCategory = ($adCategory['status'] == 1) ? $adCategory['message']['mallCategName'] : "Internal Error";
                        $adStat = $ad['mallAdStatus'];
                        $adID = $ad['mallAdID'];

                        if ($adStat == 1) {
                          $stat = '<span class="badge badge-success">Active</span>';
                        } else if ($adStat == 0) {
                          $stat = '<span class="badge badge-warning">In Review</span>';
                        } else if ($adStat == 2) {
                          $stat = '<span class="badge badge-danger">Declined</span>';
                        } else if ($adStat == 3) {
                          $stat = '<span class="badge badge-danger">Expired</span>';
                        } else if ($adStat == 4) {
                          $stat = '<span class="badge badge-danger">Deleted</span>';
                        }

                        $userDetails = $accManager->getUserDetails($ad['mallUsrID']);
                        $userDetails = $userDetails['message'];
                        $userName = $userDetails['mallUsrFirstName'] . " " . $userDetails['mallUsrLastName'];

                        $time = date("d/m/Y, H:i", $ad['mallAdPostTime']);
                        $condition = $ad['mallAdCondition'];
                        $make = $ad['mallAdMake'];
                        $model = $ad['mallAdModel'];
                        $sum = $mallUsrID . "+" . $adID;
                        if ($adStat != -1) {
                          $count++;
                          echo ' <tr>
                            <td>' . $defaultColID . '</td>
                            <td><a href="./product.php?adID=' . $adID . '">' . $adTitle . '</a></td>
                            <td>' . $adPrice . '</td>
                            <td>' . $adCategory . '</td>
                            <td>' . $stat . '</td>
                            <td><a href="user.php?id=' . $defColID . '">' . $userName . '</a></td>
                            <td><a class="text-warning mx-1" title="Delete Advert"><i onclick=' . 'show("' . $sum . '")' . ' class="fa fa-trash"></i></a> <a onclick=' . 'approveAd("' . $sum . '")' . ' class="text-success mx-1" title="Approve Advert"><i class="fas fa-thumbs-up"></i></a> <a onclick=' . 'showUsr("' . $sum . '")' . ' class="text-danger mx-1" title="Reject Advert"><i class="fas fa-thumbs-down"></i></a></td>
                            <td>' . $time . '</td>
                            <td>' . $condition . '</td>
                            <td>' . $make . '</td>
                            <td>' . $model . '</td>
    ...
    <div id="notify" class="container-fluid" style="display:none; position:fixed; bottom:0; z-index:99999; background-color:white; padding-top:15px; height:max-content; padding-bottom:0;">
      <span style="float:right; color:red; cursor:pointer;" onclick="hide('notify')">close</span>
      <div class="row justify-content-center" style="padding:20px;">
        <p class="text-center" style="color:red; display:block; width:100%;">Are you sure to delete this ad?</p><br>
        <div class="row justify-content-between">
          <form method="POST" id="deleteAd" style="width:100%;">
            <input name="adID" hidden id="adID">
            <input id="userAdID" name="userAdID" hidden>
            <?php
    ...
    <div id="notifyMsg" class="container-fluid" style="display:none; position:fixed; bottom:0; z-index:99999; background-color:white; padding-top:15px; height:max-content; padding-bottom:0;">
      <span style="float:right; color:red; cursor:pointer;" onclick="hideUsr('notifyMsg')">close</span>
      <div class="row justify-content-center" style="padding:20px;">
        <form id="declineAd" method="POST">
          <input name="userID" hidden id="inputUserID">
          <input name="adID" hidden id="adID2">
          <?php
          $admin = $_SESSION['gaijinmall_user_admin__'];
          echo '<input name="adminID" hidden id="adminID" value="' . $admin . '">';
          ?>
          <input name="message" id="message" type="text" style="padding:20px; width:100%;" placeholder="Reason for disapproval"><br><br>
  ...
  <script>
    ...
    function showUsr(sum) {
      $("#notifyMsg").fadeIn();
      const arr = sum.split("+");

      document.getElementById("inputUserID").value = arr[0];
      document.getElementById("adID2").value = arr[1];
      console.log(arr);
    ...
    function show(sum) {
      ...
      document.getElementById("userAdID").value = arr[0];
      document.getElementById("adID").value = arr[1];
    ...
    function approveAd(id) {
      ...
      $.ajax({
        ...
        if (data == 1) {
          ...
          document.getElementById("resp").innerText = "Ad approved successfully";
          // $('.toast').toast('show');
          ...
          document.getElementById("resp").innerText = "Ad status already set";
          //  $('.toast').toast('show');
          $('.toast1').fadeIn();
          setTimeout(() => {
            $('.toast1').fadeOut();
          ...
          document.getElementById("resp").innerText = "Error approving ad";
          //  $('.toast').toast('show');
          $('.toast1').fadeIn();
          setTimeout(() => {
            $('.toast1').fadeOut();
        ...
        document.getElementById("resp").innerText = "Failed to approve ad. Try again";
        // $('.toast').toast('show');
        $('.toast1').fadeIn();
        setTimeout(() => {
          $('.toast1').fadeOut();
  ...
  <script>
    $(document).ready(function() {
      ...
      $("#declineAd").submit(function(event) {
        ...
        $.ajax({
          ...
          if (data == 1) {
            ...
            document.getElementById("resp").innerText = "Ad declined successfully";
            // $('.toast').toast('show');
            ...
            document.getElementById("resp").innerText = "Error declining ad";
            //  $('.toast').toast('show');
            $('.toast1').fadeIn();
            setTimeout(() => {
              $('.toast1').fadeOut();
          ...
          document.getElementById("resp").innerText = "Failed to decline ad. Try again";
          // $('.toast').toast('show');
          $('.toast1').fadeIn();
          setTimeout(() => {
            $('.toast1').fadeOut();
      ...
      $("#deleteAd").submit(function(event) {
        ...
        $.ajax({
          ...
          if (data == 1) {
            ...
            document.getElementById("resp").innerText = "Ad deleted successfully";
            // $('.toast').toast('show');
            ...
            document.getElementById("resp").innerText = "Error deleting ad";
            //  $('.toast').toast('show');
            $('.toast1').fadeIn();
            setTimeout(() => {
              $('.toast1').fadeOut();
          ...
          document.getElementById("resp").innerText = "Failed to delete. Try again";
          // $('.toast').toast('show');
          $('.toast1').fadeIn();
          setTimeout(() => {
            $('.toast1').fadeOut();
