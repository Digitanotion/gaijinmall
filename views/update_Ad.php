...
$usrIDData = isset($_SESSION["gaijinmall_user_"]) ? $_SESSION["gaijinmall_user_"] : "null";
...
if (isset($_POST["submit_update"])) {
    if ($securityManager_ob->validateCSRF($_POST['form_token__input'])) {
        $adLocation="";
        echo $_POST["location_state"].$_POST["location_city"]."<br>";
        ...
        'mallAdLoc'=>$adLocation]);
        print_r($arrData); */
        $creatAd = $adManager->update_Ad($arrData,$adUpdate);
        if ($creatAd["status"] == 1) {
            $sys_msg['msg_type'] = 1;
...
$adUpID=$inputValidator->sanitizeItem($_GET['adUp'],"string");
...
<head>
    ...
    <title>Update Ad | Gaijinmall</title>
</head>
            ...
            <form method="post" class="" id="createAdForm" enctype="multipart/form-data">
                <div class="first">
                    ...
                    <div class="row justify-content-between mt-5">
                            ...
                            <select class="form-select p-3 select2" name="category">
                                ...
                                if ($adManagerVal["status"] === 404) { ?>
                                    ...
                                    foreach ($adManagerVal["message"] as $value) { ?>
                                        <option value="<?php echo $value["mallCategID"]; ?>" <?php echo ($value["mallCategID"]==$getCategParentInfo_UPT["mallCategID"])?"selected":"";?>><?php echo $value["mallCategName"]; ?></option>
                                <?php }
                                }
                        ...
                        <div class=" d-flex mb-1 col-md-6 col-sm-12" id="multiselec">
                            <select class="form-select p-3 select2 " name="sub_category" id="sub_category">
                                <option value="<?php echo $getCategChildInfo_UPT["mallCategID"] ?>" selected><?php echo $getCategChildInfo_UPT["mallCategName"] ?></option>
                            </select>
                        </div>
                    </div>
                    <div class="row justify-content-between mt-md-2">
                        ...
                        <div class=" d-flex mb-1 col-md-6 col-sm-12" id="multiselec">
                            <select class="form-select p-3 select2" name="location_state" id="location_state">
                            <?php 
                                $sepAdUPT_Loc=explode(".", $usrAdUpdateInfo['mallAdLoc']);
                                $buildJustCityArray="";
                                /* for ($i=1;$i<count($sepAdUPT_Loc); $i++){
                                    $buildJustCityArray.=($i==(count($sepAdUPT_Loc)-1))?$sepAdUPT_Loc[$i]:$sepAdUPT_Loc[$i]."-";
                                } */
                                ?>
                                <option value="<?php echo $sepAdUPT_Loc[1] ?>"><?php echo $sepAdUPT_Loc[1];
                                ?></option>
                                <?php
                        ...
                        <div class=" d-flex mb-1 col-md-6 col-sm-12" id="multiselec">
                            ...
                            <?php 
                                $sepAdUPT_LocState=explode(".", $usrAdUpdateInfo['mallAdLoc']);
                                $sepAdUPT_Loc=explode("-", $sepAdUPT_LocState[0]);
                                $buildJustCityArray="";
                                for ($i=0;$i<count($sepAdUPT_Loc); $i++){
                                    $buildJustCityArray.=($i==(count($sepAdUPT_Loc)-1))?$sepAdUPT_Loc[$i]:$sepAdUPT_Loc[$i]."-";
                                }
                                ?>
                                <option value="<?php echo $buildJustCityArray ?>"><?php echo $buildJustCityArray;
                                ?></option>
                                <?php
                                $getCitiesResponse = $adManager->getAllLocation();
                                if ($getCitiesResponse['status'] == 1) {
                                    foreach ($getCitiesResponse['message'] as $cityEach) {
                        ...
                        <div class=" d-flex mb-1 col-md-6 col-sm-12 mt-2" id="multiselec">
                            <div class="form-floating w-100 ">
                                <input name="mallAdTitle" id="mallAdTitle" type="text" value="<?php echo $usrAdUpdateInfo['mallAdTitle'] ?>" class="form-control" placeholder="Title" />
                                <label for="mallAdTitle">Ad Title *</label>
                            </div>
                        </div>
                        <input type="hidden" name="mallAdIDVal" id="mallAdIDVal" required value="<?php echo $adUpID; ?>">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6 button-container ">
                            <div class="large-button">
                                ...
                                <input type="hidden" name="adImgFile_UPT" id="adImgFile_UPT" value=""> 
                            </div>
                        </div>

                        <div class="col-md-6 imageDisplay" id="loadImageDisplay">
                            <?php 
                                $imageResponse=$adManager->getAdImagesByID($adUpID);
                                echo "<input type='hidden' id='json_images' class='json_images' value='".json_encode($imageResponse['message'])."'>";
                                foreach ($imageResponse['message'] as $adImages_UPT) { 
                                    echo '<div class="ha-imgUpload-each m-1" id="'.$adImages_UPT['mallMediaID'].'" style="background: url(../handlers/uploads/thumbs/'.$adImages_UPT['mallMediaName'].');">
                                            <span class="ha-del-image__item" onclick="delImageUpload('.$adImages_UPT['mallMediaID'].')">x</span>
                                            </div>';
                                     ?>
                                    <script>
                    ...
                    <div class="instruction text-secondary mt-3">
                        ...
                        <small><strong>N.B:</strong> For mobile devices select all the pictures once.</small></p>
                        <strong></strong>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button data-bs-toggle="tooltip" data-bs-placement="top" title="Welcome Tooltip" type="button" class="btn text-white sell-clr-bg-primary w-25 mb-5 mt-5 py-3 px-4 fw-bold" id="next">
                                Next
                            </button>
                        </div>
                    </div>
                ...
                <div class="mt-3 second d-none">
                    ...
                    <div class="row justify-content-between mt-5">
                        ...
                        <div class=" d-flex mb-1 col-md-12 col-sm-12 w-100 my-2" id="multiselec">
                            <div class="form-floating w-100">
                                <textarea id="mallAdDesc" required class=" form-control" placeholder="Description" name="mallAdDesc" style="resize: none; min-height:150px;"><?php echo $usrAdUpdateInfo['mallAdDesc'] ?></textarea>
                        ...
                        <div class=" d-flex mb-1 col-md-6 col-sm-12 my-2" id="multiselec">
                            <div class="input-group form-floating">
                                ...
                                <input type="number" class="form-control p-2" required placeholder="Price" id="mallAdPrice" name="mallAdPrice" value="<?php echo $usrAdUpdateInfo['mallAdPrice'] ?>">
                    ...
                    <div class="d-flex justify-content-center mt-3">
                        <div class="mt-3">
                            ...
                            <div>
                                ...
                                <button type="submit_update" name="submit_update" id="submit_update" style="font-size: 20px;" class="btn text-white mx-auto sell-clr-bg-primary w-100 mt-3  py-2 px-4 fw-bold">
                                    Update Ad
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="d-flex justify-content-center">
                        <p class="sell-terms-width sell-fs-base sell-text-color mt-2 text-muted text-center" style="font-size:12px; max-width:370px;">By clicking on Post Ad, you accept the <a href="#" class="sell-clr-primary">Terms of Use</a>, you will abide by the safety Tips, and declare that this posting does not include any probhibited items</p>
                    </div>
                </div>
            </form>
        </div>
